...
let shop = [
	...
	['Fix', 'Buys the ability to alter your current custom avatar, custom color, icon, or trainer card. (don\'t buy if you have neither)', 5],
	...
	['Custom Color', 'Buys the ability to have a custom color in every room on the server THAT WAS CREATED AT PURCHASED. PM Lmao Its BT after purchase', 50],
	['VIP', 'Buys VIP rank, which is shown in your profile. VIP Users gain the ability of infinite fixes and declare for no cost. They also get a Trainer Card and a Icon if they had not already purchased one.', 100],
];
...
shop: function(target, room, user) {
		...
		if (room.id === '' && this.broadcasting) {
			...
			let bottom = '<table><td style="' + descStyle + '">To buy an item from the shop, use /buy command.</td>';
			...
			return this.sendReply('|raw|' +
				top +
				table("Fix", "Buys the ability to alter your current custom avatar, icon, custom color, trainer card. (don\'t buy if you have neither)", 5) +
				...
				table("Declare", "Globally declare a message to the whole server! [Can be refused](A small blue message that every chatroom can see; Uses: League Advertisements, Celebrations, ETC)", 10) +
				table("League Room", "Purchases a room at a reduced rate for use with a league.  A roster must be supplied with at least 10 members for this room.", 15) +
				table("Trainer Card", "Buys a trainer card which shows information through a command. (You supply, can be refused)", 15) +
